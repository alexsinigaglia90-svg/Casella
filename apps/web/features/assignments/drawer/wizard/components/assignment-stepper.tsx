...
import { ASSIGNMENT_STEPS } from "../types";
import { isAssignmentStepValid } from "../validation";
...
export function AssignmentStepper({ step, form, onJump }: StepperProps) {
  return (
    <div className="flex items-center gap-1.5">
      {ASSIGNMENT_STEPS.map((s, i) => {
        const isActive = i === step;
        const isDone = i < step && isAssignmentStepValid(i, form);
        const isReachable =
          i <= step ||
